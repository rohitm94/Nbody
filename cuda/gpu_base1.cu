...
int main(const int argc, const char** argv) {
  ...
  for (int iter = 1; iter <= nIters; iter++) {
    ...
    // remove comments in below line to see changes in position coordinates across different time steps
    //printf("In %d timestep, position cordinates: %f\t %f\t %f\n",iter, p[0].x , p[0].y , p[0].z);
  ...
  double expected_time = float_ops_per_interaction/2.91e12;
